...
class Pair:
    ...
    def __repr__(self):
        return '点对({0}, {1})'.format(repr(self.first), repr(self.second))
    ...
    def __len__(self):
        ...
        while isinstance(second, Pair):
            ...
            second = second.second
        if second is not nil:
            raise TypeError('在不正确的列表上尝试获取长度')
        return n
...
def scheme_read(src):
    ...
    if val == '无':
        ...
        # BEGIN PROBLEM 7
        "*** YOUR CODE HERE ***"
        return Pair('引号', Pair(scheme_read(src), nil))
        # END PROBLEM 7
    elif val not in DELIMITERS:
        return val
    else:
        raise SyntaxError('不期望的牌: {0}'.format(val))
...
def buffer_input(prompt='scheme> '):
    """Return a Buffer instance containing interactive input."""
    return Buffer(tokenize_lines(InputReader(prompt)))

def buffer_lines(lines, prompt='scheme> ', show_prompt=False):
    ...
    while True:
        try:
            ...
            while src.more_on_line:
                ...
                print('字符串 :', expression)
                print('代表 :', repr(expression))
