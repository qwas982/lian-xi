...
// 2. 对于 n 个字节的符号（n>1），第一个字节的前 n 位均设为 1，第 n+1 位设为 0，后面字节的前两位设为 10。其余位用来表示符号。
...
uint32_t 获取编码UTF8的字节数(uint8_t 字节序列) {
    if ((字节序列 & 0xc0) == 0x80) {
        // byte 以 10 开头，说明是低字节
        return 0;
    } else if ((字节序列 & 0xf8) == 0xf0) {
        // byte 以 11110 开头
        return 4;
    } else if ((字节序列 & 0xf0) == 0xe0) {
        // byte 以 1110 开头
        return 3;
    } else if ((字节序列 & 0xe0) == 0xc0) {
        // byte 以 110 开头
        return 2;
    } else {
...
uint8_t 编码UTF8(uint8_t *缓冲区, int 代码点) {
    断言大写(代码点 > 0, "不能编码负值!");
    ...
    if (代码点 <= 0x7f) {
        ...
        *缓冲区++ = 0xc0 | ((代码点 & 0x7c0) >> 6);
        *缓冲区 = 0x80 | (代码点 & 0x3f);
        return 2;
    } else if (代码点 <= 0xffff) {
        *缓冲区++ = 0xe0 | ((代码点 & 0xf000) >> 12);
        *缓冲区++ = 0x80 | ((代码点 & 0xfc0) >> 6);
        *缓冲区 = 0x80 | (代码点 & 0x3f);
        return 3;
    } else if (代码点 <= 0x10ffff) {
        *缓冲区++ = 0xf0 | ((代码点 & 0x1c0000) >> 18);
        *缓冲区++ = 0x80 | ((代码点 & 0x3f000) >> 12);
        *缓冲区++ = 0x80 | ((代码点 & 0xfc0) >> 6);
        *缓冲区 = 0x80 | (代码点 & 0x3f);
        return 4;
    } else {
...
int 解码UTF8(const uint8_t *字节指针, uint32_t 长度) {
    ...
    if ((*字节指针 & 0xe0) == 0xc0) {
        ...
        代码点 = *字节指针 & 0x1f;
        字节数目 = 2;
    } else if ((*字节指针 & 0xf0) == 0xe0) {
        // 若是 3 字节的 UTF-8 (1110 开头)
        代码点 = *字节指针 & 0x0f;
        字节数目 = 3;
    } else if ((*字节指针 & 0xf8) == 0xf0) {
