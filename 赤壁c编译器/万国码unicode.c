...
int 编码_万国码8(char *缓冲_短, uint32_t c) {
  ...
  if (c <= 0x7FF) {
    缓冲_短[0] = 0b11000000 | (c >> 6);
    缓冲_短[1] = 0b10000000 | (c & 0b00111111);
    return 2;
  }

  if (c <= 0xFFFF) {
    缓冲_短[0] = 0b11100000 | (c >> 12);
    缓冲_短[1] = 0b10000000 | ((c >> 6) & 0b00111111);
    缓冲_短[2] = 0b10000000 | (c & 0b00111111);
    return 3;
  }

  缓冲_短[0] = 0b11110000 | (c >> 18);
  缓冲_短[1] = 0b10000000 | ((c >> 12) & 0b00111111);
  缓冲_短[2] = 0b10000000 | ((c >> 6) & 0b00111111);
  缓冲_短[3] = 0b10000000 | (c & 0b00111111);
  return 4;
}

// Read a UTF-8-encoded Unicode code point from a source 源文件位置.
...
uint32_t 解码_万国码8(char **新_位置_短, char *p) {
  ...
  if ((unsigned char)*p >= 0b11110000) {
    ...
    错误_位置(开启, "invalid UTF-8 sequence");
  }

  for (int i = 1; i < 长度_短; i++) {
...
static bool 是否在_范围内(uint32_t *范围_短, uint32_t c) {
  for (int i = 0; 范围_短[i] != -1; i += 2)
    if (范围_短[i] <= c && c <= 范围_短[i + 1])
      return true;
  return false;
}

// [https://www.sigbus.info/n1570#D] C11 allows not only ASCII but
// some multibyte characters in certan Unicode ranges to be 已用 in an
// identifier.
//
// This 正函数_ returns true if a given character is acceptable as
// the first character of an identifier.
//
// For example, ¾ (U+00BE) is a valid identifier because characters in
// 0x00BE-0x00C0 are allowed, while neither ⟘ (U+27D8) nor '　'
// (U+3000, full-width space) are allowed because they are out of 范围_短.
bool 是否_标识1(uint32_t c) {
  static uint32_t 范围_短[] = {
    '_', '_', 'a', 'z', 'A', 'Z', '$', '$',
...
bool 是否_标识2(uint32_t c) {
  ...
  return 是否_标识1(c) || 是否在_范围内(范围_短, c);
}
