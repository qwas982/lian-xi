...
我稍后会讲到这个字的定义。 现在只需看一下头部文件。 前4个字节是链接指针。 
...
DUP的汇编代码最终执行了一个NEXT。 这是：

	(1) 读+的地址进到%eax 			%eax指向+的代码字。
	(2) 将%esi增加4
	(3) 跳转到间接的%eax，			跳转到+的代码字内的地址。
									即- 汇编码去实现+
...
还记得我们用JMP *(%eax)跳转到这个函数吗？ 那么这样做的一个结果是，方便的%eax包含了这个代码字的地址，
所以只要在它上面加4，我们就可以得到第一个数据字的地址。 
最后在设置了%esi之后，它只是执行了NEXT，导致第一个字的运行。
*/
...
然后我们执行NEXT，由于线程代码的魔力，再次增加%esi并调用DUP。
...
在这种模式下，解释器将代码字指针追加到用户内存中（HERE变量指向用户内存的下一个空闲字节--见下面的DATA SEGMENT部分）。
...
指向前一个字的指针（来自LATEST）。	   +-- 之后，HERE指向这里，
	...
	(5) 将STATE设置为1。 
		这就进入了编译模式，因此解释器开始追加代码字到我们的部分形式的头部.
...
它所做的就是将EXIT的代码字追加到当前的定义上，并切换回立即模式（将STATE设为0）。 不久我们将看到;的实际定义，我们将看到它实际上是一个非常简单的定义，声明为IMMEDIATE。
...
/*
	...
	而<data>是栈顶的任何32位整数。 ，(COMMA) 是编译时的一个基本操作。 它用于将代码字追加到正在编译的当前字上。
*/
...
/*
	现在我们可以用CREATE定义: (COLON) 。 它只是调用CREATE，追加DOCOL（代码字），设置HIDDEN这个字，然后进入编译模式。
*/

	defword ":",1,,COLON
	.int WORD		// 获取新字的名称
	.int CREATE		// CREATE 字典条目/头部
	.int LIT, DOCOL, COMMA	// 追加 DOCOL  (代码字).
...
*/
	...
	.int LIT, EXIT, COMMA	// 追加EXIT（所以这个字会返回）.
...
/*
	...
	它将FOO的代码字追加到我们正在定义的字上（这只在编译后的代码中起作用）.
