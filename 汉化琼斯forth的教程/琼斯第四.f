...
\FORTH允许（...）作为函数定义的注释。 
\这是由一个叫做（）的IMMEDIATE词来实现的，它只是将输入的字符丢掉，直到碰到相应的）。
: ( IMMEDIATE
	1		\ 通过跟踪深度，允许嵌套的括弧。
	BEGIN
		KEY		\ 读下一个字符
...
事实上，测试也可以省略，这样你就只有一个默认情况，尽管这可能不是很有用。
	...
	一个例子（假设'q'等等 是将字母的ASCII值推到堆栈上的字）:
...
如果我们注意到所有用于定义C@++的词实际上都是汇编器的原语，那么我们可以写一个更快的（但等价的）定义，比如说:
