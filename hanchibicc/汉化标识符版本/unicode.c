...
// Read a UTF-8-encoded Unicode code point from a source 文件_小写.
...
uint32_t 解码_万国码变形格式8(char **new_pos, char *p) {
  ...
  if ((unsigned char)*p >= 0b11110000) {
    ...
    错误_在_小写(开起, "invalid UTF-8 sequence");
  }
...
// some multibyte characters in certan Unicode ranges to be 已用_小写 in an
// identifier.
//
// This 函数_全_小写 returns true if a given character is acceptable as
...
bool 是否_标识2(uint32_t c) {
  ...
  return 是否_标识1(c) || in_range(range, c);
}
